...
3. **Apply mean reversion**: a gentle pull (strength = 0.02 + 0.06 × distance) toward home values, always active, preventing parameters from drifting to extremes
4. **Apply curiosity perturbation**: every 30 seconds, a small random nudge (strength = 0.015) to a random parameter — exploration of the parameter space, biased 60% toward home values
...
6. **Budget check**: changes are rate-limited by a budget system (cost_scale = 60, restores over ~300 seconds) to prevent rapid large swings
...
See also: [Behavior Diagrams](BEHAVIOR_DIAGRAMS.md) for visual architecture walkthroughs, [Software Guide](SOFTWARE_GUIDE.md) for hotkeys and slider reference, [How It Works](HOW_IT_WORKS.md) for an accessible overview.
